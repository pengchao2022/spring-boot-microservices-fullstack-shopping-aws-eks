...
public class ElasticsearchDataInitializer {
    ...
    public void initializeElasticsearchData() {
        try {
            ...
            // 检查是否已经有数据
            long existingCount = esProductRepository.count();
            if (existingCount > 0) {
                log.info("Elasticsearch already has {} documents, skipping initialization", existingCount);
                return;
            }
            ...
            if (products.isEmpty()) {
                log.warn("No products found in database, skipping Elasticsearch initialization");
                return;
            }
            
            // 转换为 Elasticsearch 实体并保存
            ...
            log.info("Successfully initialized Elasticsearch with {} products", esProducts.size());
        } catch (Exception e) {
